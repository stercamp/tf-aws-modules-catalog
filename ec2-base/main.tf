...
resource "null_resource" "validate_bootstrap" {
  ...
  #       tags=$(aws ec2 describe-tags --filters "Name=resource-id,Values=${aws_instance.ignore_ami[0].id}" --query "Tags[?Key=='Bootstrap'].Value" --region ${local.region} --output text)
  ...
  depends_on = [aws_instance.ignore_ami]
}
